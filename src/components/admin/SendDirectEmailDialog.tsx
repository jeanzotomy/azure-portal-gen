...
export default function SendDirectEmailDialog({
 ...
 message.length > MESSAGE_MAX
 ?"text-destructive" : message.length > MESSAGE_MAX * 0.9
 ...
 <button
 key={t.id}
 type="button" onClick={() => applyTemplate(t)}
...
"flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium border-b-2 -mb-px transition-colors disabled:opacity-40",
 ...
 placeholder="Objet du message" maxLength={SUBJECT_MAX}
...
"flex flex-col items-center justify-center gap-1 rounded-md border-2 border-dashed border-border bg-muted/20 hover:bg-muted/40 hover:border-primary/50 cursor-pointer transition-colors py-4 px-3 text-center",
 ...
 disabled={submitting}
 className="p-1 rounded hover:bg-destructive/10 text-muted-foreground hover:text-destructive transition-colors disabled:opacity-40" aria-label="Retirer" >
